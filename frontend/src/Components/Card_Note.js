...
const Card_Note = (props) => {
    const { note, all_Notes, setNotes } = props;
    ...
    return (
        <div>
...

            {
                editpopup && <Popup
                    type='edit'
                    setEditPopup={setEditPopup} /* true paasss */
                    singleNote={note}
                    setNotes={setNotes} />
            }
            ...
            <Card style={{ width: '100%', borderRadius: "2rem" }} className='my-4 p-2 rounded' id="card_css"   >
                <Card.Body style={{ padding: '10px' }} >
                    <Card.Title className='card-title' style={{ padding: '4px', textAlign: 'center', alignItems: 'center' }}> {name} </Card.Title>
                    ...
                    <div class="d-flex justify-content-around " id="self_btn">
                        ...
                        <div style={{ backgroundColor: '#e63946' }} onClick={() => handleDelete(note._id)}>
                            <Delete_Icon />
                        </div>
